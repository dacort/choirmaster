...
// func (t HammerTime) MarshalJSON() ([]byte, error) {
// 	// 2013/09/20 18:40:57 +0000"" as ""2006-01-02T15:04:05Z07:00
// 	return []byte(t.Format("2006/01/02 15:04:05 +0700")), nil
// }
...
func (y *Yammer) PrintUpdatesSince(since time.Time) {
	var feed YammerFeed
	url := fmt.Sprintf("https://www.yammer.com/api/v1/messages.json?access_token=%s", y.AccessToken)
	for {
		...
		for _, message := range feed.Messages {
			if message.Created_At.Before(since) {
				getNext = false
				break
			}
			// text := fmt.Sprintf("%s: %s", feed.LookupUser(message.Sender_Id), message.GetText())
			fmt.Println(message.Created_At.Add(-7*time.Hour).Format("2006-01-02 15:04"), "\t", "Yammer")
		}
		...
		// Fetch the next set
		olderId := fmt.Sprintf("%d", feed.Messages[len(feed.Messages)-1].Id)
		url = fmt.Sprintf("https://www.yammer.com/api/v1/messages.json?access_token=%s&older_than=%s", y.AccessToken, olderId)
	}
}
