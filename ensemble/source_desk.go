...
const deskComUrl = "https://%s.desk.com"
...
func (de *DeskEntry) GetHistory(d *Desk) *CaseHistory {
	historyPath := fmt.Sprintf("/api/v2/cases/%s/history?per_page=100", de.Id())
...
func (he *HistoryEntry) GetUserName(d *Desk) (name string) {
	...
	userPath := fmt.Sprintf("/api/v2/users/%d", he.UserId())
	user := new(DeskUser)
...
func (d *Desk) FetchUpdates() (feed DeskFeed) {
	feedUrl := fmt.Sprintf("%s/api/v2/cases/search?since_updated_at=%d", d.Url, d.LastUpdate.Unix())
	fmt.Println(feedUrl)
	...
	err = dec.Decode(&feed)
	if err != nil {
		log.Printf("ERR decoding json from Desk in FetchUpdates: %s\n%s", err, resp.Body)
		return
	}
...
func (d *Desk) PrintUpdatesSince(since time.Time) {
	var feed DeskFeed
	feedUrl := fmt.Sprintf("%s/api/v2/cases/search?since_updated_at=%d&per_page=100", d.Url, since.Unix())
	for {
		req, err := http.NewRequest("GET", feedUrl, nil)
		if err != nil {
			log.Printf("ERR building request: %s", err)
		...
		err = dec.Decode(&feed)
		if err != nil {
			log.Printf("ERR decoding json from Desk in PrintUpdates: %s\n%s", err, resp.Body)
			return
		}

		counter := 1

		for _, message := range feed.Embedded.Entries {
			counter += 1
			history_items := message.GetHistory(d)

			for _, item := range history_items.Embedded.Entries {
				if item.Created_At.Before(since) || item.Type == "rule_applied" {
					continue
				}
				fmt.Println(item.Created_At.Add(-7*time.Hour).Format("2006-01-02 15:04"), "\t", "Desk")
			}

			if counter >= 45 {
				fmt.Println("Likely we hit the Desk.com rate-limit - sleeping 1 minute")
				time.Sleep(1 * time.Minute)
				counter = 0
			}
		}

		if next_link, ok := feed.Links["next"]; ok {
			historyPath := next_link.Href
			if historyPath == "" {
				break
			}
			feedUrl = fmt.Sprintf("%s%s", d.Url, historyPath)
		} else {
			break
		}
	}
...
func (d *Desk) GetUrl(path string, decode_object interface{}) {
	...
	if err != nil {
		log.Printf("ERR decoding json from Desk in GetUrl: %s\n%s", err, resp.Body)
		return
	}
